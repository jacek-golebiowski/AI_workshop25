...
class PytorchMVTECWrapper:
    """
    Wrapper do PyTorch-owego modelu, wspiera:
     - predict_proba(np.ndarray) -> np.ndarray([ [p0,p1], ... ])
     - __call__(torch.Tensor) -> torch.Tensor(logits)
    Dodane atrybuty backend/model_type oraz load_model() wymagane przez DiCE.
    """
    backend    = 'PYT'
